  ...
  const deployTLD = useCallback(async () => {
    try {
      ...
      if (window.tronWeb && window.tronWeb.ready) {
        ...
        toast.success(
          "Congratulations! Your TLD has been deployed successfully! 🎊🎉"
        );
            ...
            className={`submit-button ${
              connected && !isDeploying
                ? link
                  ? "cursor-pointer"
                  : "cursor-pointer mb-[40px]"
                : "cursor-not-allowed opacity-60 mb-[40px]"
            }`}
            onClick={showModal}
            ...
            <div className="flex items-center gap-[5px] mb-3">
              <Timer />
              {
                <span className="max-w-[800px] text-white">
                  Transaction in progress! This could take 2-3 minutes or more
                  to finalize. Feel free to relax – once it's done, we’ll
